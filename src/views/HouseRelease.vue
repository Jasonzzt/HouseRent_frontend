    ...
    <el-form ref="form" :model="sizeForm" label-width="80px" size="">
      ...
      <div style="height: 320px">
        <el-upload
            ...
            class="upload-demo"
            ref="upload"
            action="http://106.12.172.208/uploadimage"
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      if(this.houseForm.neighborhood!=''&&this.houseForm.district!=''&&this.houseForm.type!=''&&this.houseForm.layer!='层'&&
          this.houseForm.information!=''&&this.houseForm.joint!=''&&this.houseForm.area!='㎡'&&this.houseForm.cost!=''){
        this.load=true;
        ...
        uploadFiles.forEach(item => {
          const size = item.raw.size / 1024 <= 2000
          if (!size) {
            this.$message.error(`${item.raw.name}大小超过2000KB`)
            status = false
            this.load=false;
            return
          }
        ...
        this.$axios.post("http://106.12.172.208/uploadimage",form, config).then(res=>{
          ...
          if(msg==true) {
            ...
            this.$axios.post('http://106.12.172.208/gethouse', new FormData(), config).then(res => {
